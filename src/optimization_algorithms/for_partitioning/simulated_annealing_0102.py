...
class simulated_annealing_0102:
    '''
    DESCRIPTION
    ---------------
    ...
    ---------------
        ...
        arg_3(attrs):
            by default, this argument contains the following attributes:attrs={'cooling_rate':0.0001,'temperature':10000}.
    ...
    ---------------
        ...
        >>> G = nx.Graph(edgelist)
        >>> SA=simulated_annealing_0102(G,inital_partition)
    ...
    NOTE
    ------------
    The simulated annealing algorithm is well known meta heuristic algorithm that can be used in many optimization problems.

    '''
    
    def __init__(self, graph, initial_partitions=[[],[]], attrs={'cooling_rate':0.0001,'temperature':10000}):
...
class Metal:
    ...
    def generate_new_state(self): 
        ...
        while True:
            a=int(Util.random() * self.size)
            if self.state[a].value == 0:
                break    
        while True:
            b=int(Util.random() * self.size)
            if self.state[b].value == 1:
                break
                
        self.state[a].change_value()
        self.state[b].change_value()
        
        self.update_a=self.state[a]
        self.update_b=self.state[b]
    ...
    def rollback(self):
        self.update_a.change_value()
        self.update_b.change_value()
